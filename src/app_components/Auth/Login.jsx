import { SecondaryButton, SubmitPrimaryButton } from "../../atomic/Button";
import { useEffect, useState } from "react";

import AuthWrapper from "./layout";
import { Link, useNavigate } from 'react-router-dom';
import { useService, useUserSession } from '../../hooks';
import { InputAsSelector } from '../../atomic/Input';
...
const Login = () => {
    ...
    const checkForRegistrationRedirection = async (uid) => {
        try {
    ...
    */
    ...
    const handleFormLogin = async (data) => {
        try {
            setIsLoading(true);
            const _user_ = await service.loginWithEmail(data.signin_form_email, data.signin_form_password);
            setGlobalError(null);
            ...
            const _err = getErrors(err.message);
            setGlobalError(_err);
        } finally {
            setIsLoading(false);
        }
    ...
    /* ========== OR LOGIN WITH AUTH PROVIDER ========= */

    const handleProviderLogin = async (provider) => {
        setIsLoading(true);
        try {
            const _user_ = await service.loginWithProvider(provider);
            setGlobalError(null);
            checkForRegistrationRedirection(_user_.user.uid);
        } catch (error) {
            inDev && console.log("Une erreur est survenue lors de la connexion avec le service : ", error);
            const _err = getErrors(error.message);
            setGlobalError(_err);
        } finally {
            setIsLoading(false);
        }
    ...
    >
        ...
        <div className="justify-center flex w-full">
            <SecondaryButton add="max-w-xs w-full flex flex-row items-center justify-center" action={() => { handleProviderLogin("google") }}>
        ...
        { false && <div className="justify-center flex w-full">
            <SecondaryButton add="max-w-xs w-full flex flex-row items-center justify-center" action={() => { handleProviderLogin("twitter") }}>
